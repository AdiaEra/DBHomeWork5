...
with psycopg2.connect(database="clients_db", user="postgres", password="") as conn:
    with conn.cursor() as cur:
        ...
        def create_db():
            """
            Функция, создающая структуру БД (таблицы) (t)
            """
            cur.execute("""
        ...
        def get_id(current_value, find_by):
            """
            Функция, котораятнаходит ID клиента
            """
            if find_by == 'phone':
        ...
        def add_client(first_name, last_name, email):
            """
            Функция, позволяющая добавить нового клиента (cl)
            """
            cur.execute("""
        ...
        def add_phone(phone, current_value, find_by='email'):
            """
            Функция, позволяющая добавить телефон для существующего клиента (p)
            """
            cur.execute("""
        ...
        def change_client(table, column, new_value, current_value, find_by='email'):
            """
            Функция, позволяющая изменить данные о клиенте (d)
            """
        ...
        def delete_phone(find_by, current_value):
            """
            Функция, позволяющая удалить телефон для существующего клиента (dp)
            """
            cur.execute("""
        ...
        def delete_client(find_by, current_value):
            """
            Функция, позволяющая удалить существующего клиента (dc)
            """
            cur.execute("""
